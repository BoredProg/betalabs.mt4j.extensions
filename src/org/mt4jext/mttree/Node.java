...
public class Node
{
    ...
    public int addChild(Node child)
    {
        if (child.parent == null)
        {
            ...
            if (child.edge == null)
            {
                child.edge = new Edge(this, child);
            } else
            {
                child.edge.setParent(this);
            }
            child.setTree(tree);
            child.parent = this;
        ...
        {
            throw new RuntimeException("Tried to add a child that already has a parent");
        }
    }
     ...
     *
     * @return the index of this node with its parent, or -1 if there is no
     *         parent
     */
    ...
    public Edge getEdge()
    {
        ...
        {
            throw new RuntimeException("Cannot get edge to nonexistent parent");
        }
    }
    ...
    public void removeFromParent()
    {
        if (parent == null)
        {
            throw new RuntimeException("Cannot remove from nonexistent parent");
        }
     ...
     * This variant makes a default component (using
     * TreeScene.makeDefaultComponent()).
     *
     ...
     *
     *
     * Note that the component can be set to null, in order to remove the
     * component without
     * adding a new one. In that case, a default component will be generated on
     * the next
     * Tree update.
     *
     * If this node, or its Tree, is to be detached from its TreeScene (or
     * MTApplication),
     * the current component should be removed by setComponent(null) and
     * reattached/copied
     * after the move.
     *
