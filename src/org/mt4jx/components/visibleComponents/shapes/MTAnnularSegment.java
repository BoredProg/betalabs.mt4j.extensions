...
public class MTAnnularSegment extends MTCSSStylableShape
{
     ...
     * @param centerX     - the center x coordinate of the annular ring, that is
     *                    the
     ...
     * @param segments    - the number of line segments to use in simulating
     *                    arcs for
    ...
    public boolean isGeometryContainsPointLocal(Vector3D testPoint)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (this.polygons[i].isGeometryContainsPointLocal(testPoint))
            {
                return true;
            }
        }
