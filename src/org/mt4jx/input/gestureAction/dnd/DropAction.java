...
/**
 * Diffrent to DragAndDropAction, the DropAction implementation only fires
 * DropTarget.componentDropped(MTComponent droppedComponent) events. This means
 * that DropAction causes much less computing efforts because it does not look
 * for DropTargets every time the object is moved. DropAction should be used
 * instead of DragAndDropAction, if performance is too slow with
 * DradAndDropAction.
 *
 * @author Uwe Laufs
 */
public class DropAction extends AbstractDnDAction
{
     ...
     *                            if set to 'true', objects with isPickable() = 'false' will not be
     *                            checked. Set parameter to 'false' as far as
